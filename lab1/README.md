...
Количество $p$ убывает при применении правил $\texttt{p} \to \texttt{aba}$ и $\texttt{apa} \to \texttt{bb}$, значит каждое из этих правил не может быть применимо более, чем $|w|_p$ раз, поэтому бесконечная цепочка при их применении возникнуть не может.
