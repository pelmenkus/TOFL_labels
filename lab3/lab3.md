...
## LL(1) и LR(1) автоматы как работают (вроде верно)
Схема работы на грамматике $S \to aSb | ba$
![LL_LR_to_do](automat_photos/LL_and_LR_example_work.jpg)

#### LL(1)-аппроксимация
LL(1)-аппроксимация — это специально модифицированная версия исходной грамматики, которая становится LL(1), возможно изменяя язык.
LL(1)-грамматика — это грамматика, для которой можно построить LL(1)-таблицу без конфликтов.

Алгоритм:

Проверяем на левую рекурсию:
* $S \to bTSa$ - раскрывая left-to-right не можем попасть в цикл $S \to S$, так как раньше стоит нетерминал $T$
* $T \to aSTb$ - раскрывая left-to-right не можем попасть в цикл $T \to T$, так как раньше стоит нетерминал $S$
* $T \to bT$ - нет левой рекурсии

<u>PS: удаление рекурсии, например тут $A \to Aa$ делается через $A \to aA'$ и $A' \to aA' | eps$</u>

Построим $First$, $Follow$ и $Last$ множества, для этого добавим индексы чтобы сделать все терминалы различными
Это нужно чтобы:
FIRST-множества не пересекались
Устранить конфликты в LL(1)-таблице
Сделать вывод детерминированным
...
Как перейти от позиционного автомата к обычному LR-автомату:
* $.$ - переход
* Переход по нетерминалу - eps переход
* Переход по терминалу - переход в новое состояние по этому терминалу

Автомат с eps переходами

![lr0](automat_photos/lr_eps.svg)

Детерминизирую и минимизирую его

![lr0_det](automat_photos/lr_min.svg)

[Правила из пересечений грамматики с LL(1) и LR(0) тут](intersect_rules)

PS: [Отдельное спасибо](https://github.com/yoiviq/tfl/tree/main/lab3)
